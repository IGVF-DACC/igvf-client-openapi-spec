...
def fill_in_collection_template(schema_name, schema, schema_names_to_collection_names, slim_embedded_fields):
    ...
    for prop, prop_schema in schema["properties"].items():
        ...
        if 'oneOf' in prop_schema:
            ...
            if maybe_enums:
                filtered_prop_schema['enum'] = list(sorted(maybe_enums))
        if prop == '@type':
            continue
...
def generate():
    ...
    openapi_spec = generate_openapi_spec(schemas, schema_names_to_collection_names, slim_embedded_fields, version='93.0.1')
    return openapi_spec
